...
namespace LAB02
{
    ...
    public class Solver
    {
        ...
        {
            double priceWithDiscount = price - (discountPercent * price / 100);
            
            return $"Цена: {price}, скидка: {discountPercent}%, итоговая цена: {priceWithDiscount:F2}";
        }
        ...
        public static string ConvertCurrency(double rubles, string currency)
        {
            ...
            {
                case "USD":
                    newconvert = rubles / 3.0;
                    return $"{rubles} RUB = {newconvert:F2} USD";
                   
                case "EUR":
                    newconvert = rubles / 3.5;
                    return $"{rubles} RUB = {newconvert:F2} EUR";
                case "GBP":
                    newconvert = rubles / 4.0;
                    return $"{rubles} RUB = {newconvert:F2} GBP";
                default:
        ...
        public static string ArithmeticProgression(int first, int step, int n)
        {
            ...
            if (step == 0 || n <= 0)
                return "Невозможно собрать прогрессию";
            int sum = 0;
            string prog = "Прогрессия: ";
            for(int i = 0; i < n;i++)
            {
                prog += first + " ";
                sum += first;
        ...
        public static string MatrixDiagonalMax(int[,] matrix)
        {
            ...
            if (rows != cols)
                return "Максимальный элемент главной диагонали = 0";
            int max = matrix[0, 0];
            ...
            return $"Максимальный элемент главной диагонали = {max}";
        }
    }
}
